...
void dump_escaped_char(uint8_t ch)
{
  if (ch >= 0x20 && ch < 0x7f)
    printf("%c", ch);
  else
...
public:
  ...
  virtual void dump_rawdata_as_js(std::string prefix)
  {
    for (size_t i = 0; i < rawdata.size(); i += sizeof(T)) {
      ...
      std::cout << "]\n";
      if (T::do_dump_as_string) {
	std::cout << prefix << "." << data.name.val() << "[" << i << ":" << i+sizeof(T) << "] = ";
	vec[i/sizeof(T)].dump_as_js_string();
	std::cout << "\n";
      }
    }
  }
...
class ADTMaybeAscizProp : public ADTProp {
public:
  ADTMaybeAscizProp(ADTProp prop)
    : ADTProp(prop)
  {
  }
  ...
  virtual void dump_rawdata_as_js(std::string prefix)
  {
    for (size_t i = 0; i < rawdata.size(); i++) {
      std::cout << prefix << "." << data.name.val() << "[" << i << ":" << (i+1) << "] = [";
      printf("0x%02x /* '%c' */", (uint8_t)rawdata[i], (uint8_t)rawdata[i]);
      std::cout << "]\n";
    }
  }
};

class ADTFunctionProp : public ADTClassArrayProp<uint32_le_t> {
public:
  ADTFunctionProp(ADTProp prop)
    : ADTClassArrayProp(prop)
  {
  }

  virtual void dump_rawdata_as_js(std::string prefix)
  {
    for (size_t i = 0; i < rawdata.size(); i += sizeof(uint32_le_t)) {
      std::cout << prefix << "." << data.name.val() << "[" << i << ":" << i+sizeof(uint32_le_t) << "] = [";
      vec[i/sizeof(uint32_le_t)].dump_as_js();
      vec[i/sizeof(uint32_le_t)].dump_as_js_comment();
      std::cout << "]\n";
      std::cout << prefix << "." << data.name.val() << "[" << i << ":" << i+sizeof(uint32_le_t) << "] = [";
      vec[i/sizeof(uint32_le_t)].dump_as_js_string();
      std::cout << "]\n";
    }
  }
};

class ADTFakeAcpiProp : public ADTProp {
public:
  ADTFakeAcpiProp(ADTProp prop)
    : ADTProp(prop)
  {
  }
...
ADTProp *ADTProp::parse()
{
  ...
  if (data.name.val() == "manufacturer")
    return new ADTStringsProp(*this);
  if (data.name.val() == "model")
    return new ADTStringsProp(*this);
  if (data.name.val() == "serial-number")
    return new ADTStringsProp(*this);
  if (data.name.val() == "secure-root-prefix")
    return new ADTStringsProp(*this);
  if (data.name.val() == "region-info")
    return new ADTStringsProp(*this);
  if (data.name.val() == "compatible")
    return new ADTStringsProp(*this);
  if (data.name.val() == "platform-name")
    return new ADTStringsProp(*this);
  if (data.name.val() == "AAPL,phandle")
    return new ADTArrayProp<uint32_t>(*this);
  if (data.name.val() == "iommu-parent")
    return new ADTArrayProp<uint32_t>(*this);
  if (data.name.val() == "interrupt-parent")
    return new ADTArrayProp<uint32_t>(*this);
  if (data.name.val() == "#size-cells")
    return new ADTArrayProp<uint32_t>(*this);
  if (data.name.val() == "#address-cells")
    return new ADTArrayProp<uint32_t>(*this);
  if (data.name.val() == "#address-cels") /* A typo in the ADT? */
    return new ADTArrayProp<uint32_t>(*this);
  if (data.name.val() == "clock-gates")
    return new ADTArrayProp<uint32_t>(*this);
  if (data.name.val() == "clock-ids")
    return new ADTArrayProp<uint32_t>(*this);
  if (data.name.val() == "interrupts")
    return new ADTArrayProp<uint32_t>(*this);
  if (data.name.val() == "power-gates")
    return new ADTArrayProp<uint32_t>(*this);
  if (data.name.val() == "reg")
    return new ADTClassArrayProp<uint32_le_t>(*this);
  if (data.name.val() == "nvram-proxy-data")
    return new ADTBlobProp(*this);
  if (data.name.val() == "bluetooth-taurus-calibration")
    return new ADTBlobProp(*this);
  if (data.name.val() == "bluetooth-taurus-calibration-bf")
    return new ADTBlobProp(*this);
  if (data.name.val() == "multi-touch-calibration")
    return new ADTBlobProp(*this);
  if (data.name.val() == "wifi-calibration-msf")
    return new ADTBlobProp(*this);
  if (data.name.val() == "function-switch_config")
    return new ADTFakeAcpiProp(*this);
  if (data.name.val() == "devices" && data.len > 1024)
    return new ADTClassArrayProp<PmgrDevice>(*this);
  if (data.name.val() == "ps-regs")
    return new ADTClassArrayProp<PmgrPsReg>(*this);
  if (data.name.val() == "energy-counters")
    return new ADTClassArrayProp<PmgrEnergyCounter>(*this);
  if (data.name.val() == "power-domains")
    return new ADTClassArrayProp<PmgrPowerDomain>(*this);
  if (data.name.val() == "perf-domains")
    return new ADTClassArrayProp<PmgrPerfDomain>(*this);
  if (data.name.val() == "location-id")
    return new ADTStringsProp(*this);
  if (data.name.val() == "location-name")
    return new ADTStringsProp(*this);
  if (data.name.val() == "device_type")
    return new ADTStringsProp(*this);
  if (strncmp(data.name.val().c_str(), "function-", strlen("function-")) == 0)
    return new ADTFunctionProp(*this);
  ...
  for (size_t i = 0; i < rawdata.size(); i++) {
    ...
    if (ch)
      non_zero = true;
    if ((ch < 0x20 || ch >= 0x7f) && (ch || i < rawdata.size() - 1))
      non_ascii = true;
  }
  if (!non_zero)
    return new ADTZeroProp(*this);
  if (!non_ascii)
    return new ADTMaybeAscizProp(*this);
  if (rawdata.size() % 4 == 0)
    return new ADTClassArrayProp<uint32_le_t>(*this);
  return new ADTProp(*this);
}
